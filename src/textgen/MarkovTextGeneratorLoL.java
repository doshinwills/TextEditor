...
public class MarkovTextGeneratorLoL implements MarkovTextGenerator {
	...
	public void train(String sourceText) {
		...
		for (String word : tokens) {
			...
			for (ListNode listNode : wordList) {
				if (listNode.getWord() != null && prevWord != null && prevWord.equals(listNode.getWord())) {
					listNode.addNextWord(word);
					newElement = false;
				}
			}
			if (prevWord != null && newElement) {
	...
	public String generateText(int numWords) {
		...
		while(numWords > 1) {
			for (ListNode listNode : wordList) {
				if (listNode.getWord() != null && currWord != null && currWord.equals(listNode.getWord())) {
					currWord = listNode.getRandomNextWord(rnGenerator);
