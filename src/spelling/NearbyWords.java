...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		for(char letter = 'a'; letter <= 'z'; letter++) {
			for(int index = 0; index <= s.length(); index++) {
				if(wordsOnly) {
					if(dict.isWord(s.substring(0, index) + letter + s.substring(index)))
							currentList.add(s.substring(0, index) + letter + s.substring(index));
				} else {
					currentList.add(s.substring(0, index) + letter + s.substring(index));
				}
			}
	...
	public void deletions(String s, List<String> currentList, boolean wordsOnly ) {
		for(int index = 1; index <= s.length(); index++) {
			if(wordsOnly) {
				if(dict.isWord(s.substring(0, index-1) + s.substring(index)))
						currentList.add(s.substring(0, index - 1) + s.substring(index));
			} else {
				currentList.add(s.substring(0, index-1) + s.substring(index));
			}
		}
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while(queue.size() != 0 && retList.size() < numSuggestions) {
			...
			List<String> strings = distanceOne(curr, false);
